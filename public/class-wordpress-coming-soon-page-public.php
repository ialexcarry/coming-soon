 ...
 * @subpackage Wordpress_Coming_Soon_Page/public
 */
 ...
 */
class Wordpress_Coming_Soon_Page_Public {
	...
	public function enqueue_scripts() {
		 ...
		 *
		 * An instance of this class should be passed to the run() function
		 * defined in Wordpress_Coming_Soon_Page_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Wordpress_Coming_Soon_Page_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */
	 ...
	 * @name mwb_wcsp_display_coming_soon
	 * @since    1.0.0
	 */
	public function mwb_wcsp_display_coming_soon()
	{
      ...
      global $pagenow;
        $wcsp_display = get_option( "wcsp_enable_imgurl", false );
        if(empty( $wcsp_display)) 
        {
        	 $wcsp_display='bg2';
        }
      if($pagenow!='wp-login.php'&&!is_admin()&&!current_user_can('manage_options'))
      {
       require_once WCSP_dir_path.'admin/template/'.$wcsp_display.'/'.$wcsp_display.'.php';
           die(); 
	  }
   }
